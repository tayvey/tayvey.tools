using Demo.WebApi.Business.Interfaces;
using Demo.WebApi.Model;
using SqlSugar;
using Tayvey.Tools.Attributes;
using Tayvey.Tools.Enums;
using Tayvey.Tools.Interfaces;

namespace Demo.WebApi.Business.Services;

/// <summary>
/// MSSQL DEMO业务
/// </summary>
[TvAutoDI(TvAutoDILifeCycle.Scoped)]
public class MssqlDemo(
    ITvSqlSugarRepository<MssqlDemoEntity> mssqlDemo) : IMssqlDemo
{
    /// <summary>
    /// MSSQL DEMO
    /// </summary>
    private readonly ITvSqlSugarRepository<MssqlDemoEntity> _mssqlDemo = mssqlDemo;

    /// <summary>
    /// 获取所有Name
    /// </summary>
    /// <returns></returns>
    public async Task<List<string>> GetNamesAsync()
    {
        var data = await _mssqlDemo.GetListAsync(i => true);
        return data.Select(i => i.Name).ToList();
    }
}